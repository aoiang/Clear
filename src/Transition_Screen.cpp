...
int Transition_Screen::animate_text(sf::Text text) {
    int frame_count = 0;
    //count = 0;
    sf:: Event event;
    sf::Clock clock;
    ...
    while (frame_count <= 255) {
        while (window->pollEvent(event)) {
            ...
            if (event.type == sf::Event::KeyPressed) {return SCREEN_MAINMENU;}
        }
...
int Transition_Screen::run() {
    ...
    while (true) {
        while (window->pollEvent(Event)) {
            if (Event.type == sf::Event::Closed) {return EXIT_GAME;}
            else if (Event.type == sf::Event::KeyPressed) {return SCREEN_GAMEVIEW;}
        }
        //window->clear(sf::Color::Black);
        //window->draw(*message);
        window->display();
        if (clock.getElapsedTime().asSeconds() > 0.2f) {return SCREEN_GAMEVIEW;}
    }
}
